...
async function generateProductCardImage(productUrl: string, browserlessKey: string, showHighlights: boolean) {
  let browser;
  try {
    console.log("🚀 Starting for:", productUrl);
    ...
    if (!clipRegion) {
      console.error("❌ #ppd NOT FOUND");
      throw new Error("Could not find product details container (#ppd)");
    }

    console.log("📸 Capturing...");
    await new Promise(r => setTimeout(r, 800)); // وقت بسيط عشان الـ UI يثبت
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`📩 ASIN: ${asin} | Device: ${deviceId}`);
